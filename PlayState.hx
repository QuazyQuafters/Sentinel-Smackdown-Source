...
class PlayState extends MusicBeatState
{
	...
	public var dialogue:Array<String> = ['dad:blah blah blah', 'dadconfused:blah blah blah', 'dadangry:blah blah blah', 'bf:coolswag', 'synapse:coolswag', 'bfshock:coolswag', 'bfdead:coolswag', 'bfconfused:coolswag'];
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'permafreshie':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/permafreshie/dialog'));
			case 'gripped':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/gripped/dialog'));
			case 'synapse':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/synapse/dialog'));
		}
				...
				case 'sentinel':
					{
						defaultCamZoom = 0.9;
						curStage = 'sentinel';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('funkin_ashiin_bg_art'));
						bg.antialiasing = true;
						...
						bottomBoppers = new FlxSprite(-375, 300);
						bottomBoppers.frames = Paths.getSparrowAtlas('funkin_ashiin_bg_characters', 'shared');
						bottomBoppers.animation.addByPrefix('bop', 'background limo pink', 24, false);
						if(FlxG.save.data.antialiasing)
							{
								bottomBoppers.antialiasing = true;
							}
		...
		switch (SONG.player2)
		{
			...
			case 'freshie':
				...
				dad.y += 225;
			case 'synapse':
				FlxTween.tween(dad, {x: 100, y: 200}, 3, {type : FlxTweenType.PINGPONG, loopDelay: 0});
			case 'senpai':
				dad.x += 150;
				dad.y += 360;
		...
		if (boyfriend.curCharacter == 'bf-fisch')
		{
			healthBar.createFilledBar(0xFFFF0000, 0xFF00FFE5);
		}
		else
		{
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var synapse:FlxSprite = new FlxSprite();
		synapse.frames = Paths.getSparrowAtlas('synapse');
		synapse.animation.addByPrefix('idle', 'synapse loading', 24, false);
		synapse.setGraphicSize(Std.int(synapse.width * 1));
		synapse.scrollFactor.set();
		synapse.updateHitbox();
		synapse.screenCenter();
		...
		if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'gripped')
		{
			remove(black);

			if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'synapse')
			{
				add(red);
			}
		}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					{
						add(synapse);
						FlxG.sound.play(Paths.sound('quazysLoudAssKeyboard'));
						synapse.alpha = 0;
						new FlxTimer().start(0.1, function(swagTimer:FlxTimer)
						{
							...
							{
								synapse.animation.play('idle');
								FlxG.sound.play(Paths.sound('ANGRY'), 1, false, null, true, function()
								{
									new FlxTimer().start(3.2, function(deadTime:FlxTimer)
									{
										remove(synapse);
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if(SONG.player2 == 'synapse' && curBeat >= 96 && health < 0.2)
					{
	...
	override function beatHit()
	{
		...
		if (curSong == 'Synapse' && dad.curCharacter == 'synapse')
		{
			if (curBeat == 96)
				FlxG.sound.play(Paths.sound('clickagain'));
			if (curBeat == 132)
				FlxG.sound.play(Paths.sound('clickagain'));
			if (curBeat == 175)
				FlxG.sound.play(Paths.sound('clickagain'));
		...
		if (curSong == 'Gripped')
		{
			if (curBeat > 71 && curBeat < 74)
			{
				FlxTween.tween(FlxG.camera, {zoom: 1.5}, 0.7, {ease: FlxEase.expoOut,});
			}
			if (curBeat == 72)
			{
				dad.playAnim('agility', true);
			}
		}
		if (curSong == 'Synapse')
		{
			if (curBeat > 547 && curBeat < 549)
			{
				dad.playAnim('BANNED', true);
			}
			if (curBeat == 550)
			{
				remove(dad);
